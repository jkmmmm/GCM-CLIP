...
class CMCLIPLoss(ClipLoss):
    def __init__(
        ...
        self.supervise_loss = SupervisedContrastiveLoss(
            temperature=temperature,
            base_temperature=base_temperature,
            scale_by_temperature=scale_by_temperature
        )

        input_dim = 512  # 输入特征维度为512
        self.image_location_proj_head = Projection_head(input_dim=512, output_dim=input_dim//4, hidden_dim=2048)
        self.text_location_proj_head = Projection_head(input_dim=512, output_dim=input_dim//4, hidden_dim=2048)
        
        self.image_health_proj_head = Projection_head(input_dim=input_dim//4, output_dim=input_dim//16, hidden_dim=2048)
        self.text_health_proj_head = Projection_head(input_dim=input_dim//4, output_dim=input_dim//16, hidden_dim=2048)
        
    # def dist_loss(self, teacher_logits, student_logits):
    #     teacher_softmax = teacher_logits.softmax(dim=1)
    #     student_log_softmax = student_logits.log_softmax(dim=1)
    #     kl_div = F.kl_div(student_log_softmax, teacher_softmax, reduction='batchmean')
    #     return kl_div
    def dist_loss(self, teacher_logits, student_logits):
        return -(teacher_logits.softmax(dim=1) * student_logits.log_softmax(dim=1)).sum(dim=1).mean(dim=0)
    def mse_loss(self, teacher_feature, student_feature):
        return F.mse_loss(teacher_feature, student_feature)
    def forward(self, image_features, text_features, location, category, implicit_category_0, implicit_category_1, epoch, logit_scale, output_dict=False):
    # def forward(self, image_features, text_features, teacher_image_features, teacher_text_features, location, category, implicit_category_0, implicit_category_1, epoch, logit_scale, output_dict=False):
        device = image_features.device
        logits_per_image, logits_per_text = self.get_logits(image_features, text_features, logit_scale)
        # logits_per_teacher_image, logits_per_teacher_text = self.get_logits(teacher_image_features, teacher_text_features, logit_scale)
        ...
        # mse损失
        # mse_image_loss = self.mse_loss(teacher_image_features, image_features)
        # mse_text_loss = self.mse_loss(teacher_text_features, text_features)
        # mse_loss = (mse_image_loss + mse_text_loss) / 2
        # 自监督蒸馏
        # distill_loss = (
        #     self.dist_loss(logits_per_teacher_image, logits_per_image) + 
        # self.dist_loss(logits_per_teacher_text, logits_per_text)
        # ) / 2

        #监督对比学习，先学部位---→再学疾病分类
        text_location_proj = self.text_location_proj_head(text_features)
        image_location_proj = self.image_location_proj_head(image_features)

        location_loss = self.supervise_loss(image_location_proj, text_location_proj, location, logit_scale)
        
        text_health_proj = self.text_health_proj_head(text_location_proj)
        image_health_proj = self.image_health_proj_head(image_location_proj)

        health_loss = self.supervise_loss(image_health_proj, text_health_proj, category, logit_scale)

        
        if epoch > 0:
        # 隐式监督对比学习
            implicit_category0_loss = self.supervise_loss(image_features, text_features, implicit_category_0, logit_scale)
            implicit_category1_loss = self.supervise_loss(image_features, text_features, implicit_category_1, logit_scale)
        else:
            implicit_category0_loss = torch.zeros_like(contrastive_loss, requires_grad=True)
            implicit_category1_loss = torch.zeros_like(contrastive_loss, requires_grad=True)
        total_loss = contrastive_loss + 10 * location_loss + 10 * health_loss + 10 * implicit_category0_loss + 10 * implicit_category1_loss
        
        return {
            "contrastive_loss": contrastive_loss,
            # "distill_loss": 0.1 * distill_loss,
            "location_loss": 10 * location_loss,
            "health_loss": 10 * health_loss,
            "implicit_category0_loss": 10 * implicit_category0_loss,
            "implicit_category1_loss": 10 * implicit_category1_loss
