...
class cls_CMCLIP(nn.Module):
    ...
    def __init__(
        ...
        # 分类头，线性映射
        self.loc_class_head = nn.linear(embed_dim, 10)
        self.health_class_head = nn.linear(embed_dim, 20)
        self.combine_class_head = nn.linear(embed_dim, 200)
    ...
    def forward(
        ...
        loc_cls = self.loc_class_head(image_features)
        health_cls = self.health_class_head(image_features)
        combine_cls = self.combine_class_head(image_features)
        ...
        if self.logit_bias is not None:
            return image_features, text_features, loc_cls, health_cls, combine_cls, self.logit_scale.exp(), self.logit_bias
        return image_features, text_features, loc_cls, health_cls, combine_cls, self.logit_scale.exp()
