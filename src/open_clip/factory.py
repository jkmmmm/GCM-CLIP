...
from .model import CLIP, CustomTextCLIP, CMCLIP, cls_CMCLIP,convert_weights_to_lp, convert_to_custom_text_state_dict,\
    resize_pos_embed, get_cast_dtype, resize_text_pos_embed, set_model_preprocess_cfg
from .coca_model import CoCa
from .loss import ClipLoss, DistillClipLoss, CoCaLoss, SigLipLoss, HardNegativeLoss, CMCLIPLoss, cls_CMCLIPLoss
...
def load_state_dict(
    ...
    if str(checkpoint_path).endswith(".safetensors"):
        ...
        try:
            checkpoint = torch.load(checkpoint_path, map_location=device, weights_only=weights_only, strict=strict)
        except TypeError:
...
def create_model(
        ...
        force_CMCLIP: bool = False,
        force_cls_CMCLIP: bool = False,
    ...
    if custom_text:
        ...
        print("-"*50)
        print("Using CMCLIP model")
        print("-"*50)
        model = CMCLIP(**model_cfg, cast_dtype=cast_dtype)
        model.to(device=device)
    elif force_cls_CMCLIP:
        print("-"*50)
        print("Using cls_CMCLIP model")
        print("-"*50)
        model = cls_CMCLIP(**model_cfg, cast_dtype=cast_dtype)
    else:
    ...
    if pretrained:
        ...
        if checkpoint_path:
            ...
            # 加载检查点到 CPU
            state_dict = load_state_dict(checkpoint_path, device=device, weights_only=load_weights_only, strict=False)
            
            # 检测并转换第三方 state_dicts
            #state_dict = convert_state_dict(model, state_dict)
            
            # 特殊处理：WordCLIPGrounder 模型
            if force_CMCLIP or force_cls_CMCLIP:
                # 1. 记录初始 logit_scale 值
                #original_logit_scale = model.logit_scale.clone().detach().item()
                ...
                for k, v in state_dict.items():
                    # 跳过 logit_scale 相关参数
                    # if k in ["logit_scale"]:
                    #     logging.warning(f"Skipping parameter: {k} (special handling)")
                    #     continue
                    ...
                    if k in model_state and v.shape == model_state[k].shape:
                        ...
                        logging.warning(f"Skipping incompatible parameter: {k} (shape mismatch)")
                
                # 3. 加载兼容权重
                model.load_state_dict(compatible_weights, strict=False)
                
                # 4. 记录加载后的 logit_scale 值
                # new_logit_scale = model.logit_scale.item()

                
                # logging.info(f"Logit scale values - Original: ({original_logit_scale:.4f}, exp={np.exp(original_logit_scale):.4f}), "
                #              f"New: ({new_logit_scale:.4f}, exp={np.exp(new_logit_scale):.4f})")
...
def create_loss(args):
    if args.distill:
    ...
    elif args.CMCLIP_loss:
        print("-"*50)
        print("Using CMCLIP loss")
        print("-"*50)
        return  CMCLIPLoss(
            local_loss=args.local_loss,
            gather_with_grad=args.gather_with_grad,
        ...
        print("-"*50)
        print("Using cls_CMCLIPLoss loss")
        print("-"*50)
        return  cls_CMCLIPLoss(
            local_loss=args.local_loss,
            gather_with_grad=args.gather_with_grad,
            cache_labels=True,
            rank=args.rank,
...
def create_model_and_transforms(
    ...
    model = create_model(
        ...
        force_CMCLIP=force_CMCLIP,
        force_cls_CMCLIP=force_cls_CMCLIP,
